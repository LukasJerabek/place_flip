...
@login_required
def result():
    chosen = db.session.query(Place).filter_by(done = 0).order_by(func.random()).first()
    if chosen == None:
        chosen = 'No places in the database!'
    else:
        chosen = db.session.query(Place).filter_by(done = 0).order_by(func.random()).first().where
